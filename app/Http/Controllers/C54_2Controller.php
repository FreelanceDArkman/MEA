...
class C54_2Controller extends Controller
{
    ...
    public  function  getData($ArrParam){
        ...
        $query = "";
        if($NEWS_CATE_ID > 0){
            $query =  "SELECT nt.FAQ_CATE_ID As FAQ_CATE_ID , nt.FAQ_QUESTION_ID AS FAQ_QUESTION_ID,nc.FAQ_CATE_NAME AS FAQ_CATE_NAME,nt.FAQ_QUESTION_DETAIL AS FAQ_QUESTION_DETAIL,nt.FAQ_ANSWER_ID AS FAQ_ANSWER_ID, nt.FAQ_ANSWER_DETAIL AS FAQ_ANSWER_DETAIL,nt.FAQ_TOPIC_FLAG AS FAQ_TOPIC_FLAG,nt.START_DATE AS START_DATE,nt.EXPIRE_DATE AS EXPIRE_DATE FROM TBL_FAQ_TOPIC nt INNER JOIN TBL_FAQ_CATE nc ON nc.FAQ_CATE_ID=nt.FAQ_CATE_ID WHERE nt.FAQ_CATE_ID = '".$NEWS_CATE_ID."' AND nt.FAQ_TOPIC_FLAG = '".$NEWS_TOPIC_FLAG."'  ORDER BY nt.CREATE_DATE DESC  OFFSET ".$PageSize." * (".$PageNumber." - 1) ROWS FETCH NEXT ".$PageSize." ROWS ONLY OPTION (RECOMPILE)";
        }else{
            $query =  "SELECT nt.FAQ_CATE_ID As FAQ_CATE_ID , nt.FAQ_QUESTION_ID AS FAQ_QUESTION_ID,nc.FAQ_CATE_NAME AS FAQ_CATE_NAME,nt.FAQ_QUESTION_DETAIL AS FAQ_QUESTION_DETAIL,nt.FAQ_ANSWER_ID AS FAQ_ANSWER_ID, nt.FAQ_ANSWER_DETAIL AS FAQ_ANSWER_DETAIL,nt.FAQ_TOPIC_FLAG AS FAQ_TOPIC_FLAG,nt.START_DATE AS START_DATE,nt.EXPIRE_DATE AS EXPIRE_DATE FROM TBL_FAQ_TOPIC nt INNER JOIN TBL_FAQ_CATE nc ON nc.FAQ_CATE_ID=nt.FAQ_CATE_ID  WHERE  nt.FAQ_TOPIC_FLAG = '".$NEWS_TOPIC_FLAG."' ORDER BY nt.CREATE_DATE DESC  OFFSET ".$PageSize." * (".$PageNumber." - 1) ROWS FETCH NEXT ".$PageSize." ROWS ONLY OPTION (RECOMPILE)";
        }
