...
class HomeController extends Controller
{
    ...
    public function  GetFeed($feedby){
        ...
        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_TOPIC_FLAG','=',0)->where('NEWS_CATE_ID','1')->orWhere('NEWS_CATE_ID','2')->orderBy('create_date', 'desc')->paginate(6);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_TOPIC_FLAG','=',0)->where('NEWS_CATE_ID','1')->orWhere('NEWS_CATE_ID','2')->orderBy('VIEW_STAT', 'desc')->paginate(6);
        }
