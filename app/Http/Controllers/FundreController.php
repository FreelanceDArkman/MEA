...
//
    ...
    public function  GetFeed($feedby){
        ...
        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','6')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('create_date', 'desc')->paginate(10);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','6')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('VIEW_STAT', 'desc')->paginate(10);
        }

        if($feedby == 3){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','6')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('DL_STAT', 'desc')->paginate(10);
        }
