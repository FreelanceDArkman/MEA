...
//        $netasset  = DB::table('tbl_news_topic')->where('NEWS_CATE_ID','3')->orderBy('create_date', 'desc')->paginate(10);
    ...
    public function  GetFeed($feedby){
        ...
        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','3')->orderBy('create_date', 'desc')->paginate(10);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','3')->orderBy('VIEW_STAT', 'desc')->paginate(10);
        }

        if($feedby == 3){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','3')->orderBy('DL_STAT', 'desc')->paginate(10);
        }
