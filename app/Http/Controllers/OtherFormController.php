...
class OtherFormController extends Controller
{
    ...
    {
        $this->pageSetting( [
            'title' => 'แบบฟอร์มและใบคำร้องอื่นๆ | MEA FUND'
        ] );

//        $sql = "SELECT * FROM tbl_news_topic WHERE NEWS_CATE_ID = 14 ORDER BY create_date DESC";
//        $netasset = DB::select(DB::raw($sql));

//        $netasset  = DB::table('tbl_news_topic')->where('NEWS_CATE_ID','14')->orderBy('create_date', 'desc')->paginate(10);

        $netasset = $this->GetFeed(1);
        return view('frontend.pages.5p3')->with(['netasset' => $netasset,'sortby'=>1]);
    }
    ...
    public function getIndexByID($id)
    {
        ...
        return view('frontend.pages.5p3')->with(['netasset' => $netasset,'sortby'=>$id]);
    }
