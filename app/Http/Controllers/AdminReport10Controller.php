...
LEFT OUTER JOIN ( SELECT SUM(ios.COUNT_FIRST_ACTIVATED) as IOSCOUNT, CONVERT(VARCHAR(10), ios.TIMESTAMP, 120) AS iosTIme FROM  TBL_MOBILE_ACTIVATED_STAT ios WHERE ios.OS = 'iOS'
GROUP BY ios.TIMESTAMP ) ioss ON ioss.iosTIme = CONVERT(VARCHAR(10), st.TIMESTAMP, 120)
LEFT OUTER JOIN ( SELECT SUM(andr.COUNT_FIRST_ACTIVATED) as AndrCOUNT, CONVERT(VARCHAR(10), andr.TIMESTAMP, 120)  AS andrTIme FROM  TBL_MOBILE_ACTIVATED_STAT andr WHERE andr.OS = 'Android' GROUP BY andr.TIMESTAMP ) andrr ON andrr.andrTIme = CONVERT(VARCHAR(10), st.TIMESTAMP, 120)";
