...
class NeesfundController extends Controller
{
    public function getIndex()
    {
        ...
        return view('frontend.pages.7p2')->with(['netasset' => $netasset,'sortby'=>1]);
    ...
    public function getIndexByID($id)
    {
        ...
        return view('frontend.pages.7p2')->with(['netasset' => $netasset,'sortby'=>$id]);
    }

    public function  GetFeed($feedby){
        $today = new Date();
        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','2')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('create_date', 'desc')->paginate(6);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','2')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('VIEW_STAT', 'desc')->paginate(6);
        }

        if($feedby == 3){
            return DB::table('tbl_news_topic')->where('START_DATE','<=',get_date_sql($today))->where('EXPIRE_DATE','>=',get_date_sql($today))->where('NEWS_CATE_ID','2')->where('NEWS_TOPIC_FLAG','=',0)->orderBy('DL_STAT', 'desc')->paginate(6);
        }
