...
//        $netasset = DB::select(DB::raw($sql));
    ...
    public function  GetFeed($feedby){
        ...
        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','10')->orderBy('create_date', 'desc')->paginate(6);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','10')->orderBy('VIEW_STAT', 'desc')->paginate(6);
        }

        if($feedby == 3){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','10')->orderBy('DL_STAT', 'desc')->paginate(6);
        }
