...
class BoardController extends Controller
{
    ...
    {
        $this->pageSetting( [
            'title' => 'คำสั่งแต่งตั้งคณะกรรมการ | MEA FUND'
        ] );

//        $sql = "SELECT * FROM tbl_news_topic WHERE NEWS_CATE_ID = 5 ORDER BY create_date DESC";
//        $netasset = DB::select(DB::raw($sql));

//        $netasset  = DB::table('tbl_news_topic')->where('NEWS_CATE_ID','5')->orderBy('create_date', 'desc')->paginate(10);

        $netasset = $this->GetFeed(1);
        return view('frontend.pages.2p3')->with(['netasset' => $netasset,'sortby'=>1]);
    }
    ...
    public function getIndexByID($id)
    {
        ...
        return view('frontend.pages.2p3')->with(['netasset' => $netasset,'sortby'=>$id]);
    }

    public function  GetFeed($feedby){

        if($feedby == null || $feedby == 1){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','5')->orderBy('create_date', 'desc')->paginate(10);
        }

        if($feedby == 2){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','5')->orderBy('VIEW_STAT', 'desc')->paginate(10);
        }

        if($feedby == 3){
            return DB::table('tbl_news_topic')->where('NEWS_CATE_ID','5')->orderBy('DL_STAT', 'desc')->paginate(10);
        }
