...
//        $where = " WHERE (focus.EMP_ID = '".$emp_id."' OR em.DEP_SHT  = '".$depart."' OR new.PLAN_ID = '".$plan."' OR new.MODIFY_DATE BETWEEN '".$date_start."' AND '".$date_end."')";
    ...
    public  function  DataSource($ArrParam, $IsCase, $ispageing= true){
        ...
        $query="SELECT focus.EMP_ID As EMP_ID,em.FULL_NAME AS FULL_NAME,em.DEP_SHT AS DEP_SHT , new.USER_SAVING_RATE AS rate_new,old.USER_SAVING_RATE as rate_old, new.CHANGE_SAVING_RATE_DATE AS modify_new , new.EFFECTIVE_DATE AS effec_new,new.MODIFY_BY AS new_modify_by
...
OUTER APPLY (SELECT TOP 1 ol.USER_SAVING_RATE FROM TBL_USER_SAVING_RATE ol WHERE ol.EMP_ID = focus.EMP_ID AND ol.CHANGE_SAVING_RATE_DATE <new.CHANGE_SAVING_RATE_DATE) AS old ";
